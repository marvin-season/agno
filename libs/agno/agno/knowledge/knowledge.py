...
from agno.knowledge.remote_content.config import GcsConfig, GitHubConfig, RemoteContentConfig, S3Config, SharePointConfig
...
class Knowledge:
    ...
    async def _load_from_remote_content_async(
        ...
        if isinstance(remote_content, S3Content):
            await self._load_from_s3_async(content, upsert, skip_if_exists, config)

        elif isinstance(remote_content, GCSContent):
            await self._load_from_gcs_async(content, upsert, skip_if_exists, config)

        elif isinstance(remote_content, SharePointContent):
            await self._load_from_sharepoint_async(content, upsert, skip_if_exists, config)

        elif isinstance(remote_content, GitHubContent):
            await self._load_from_github_async(content, upsert, skip_if_exists, config)

        else:
            log_warning(f"Unsupported remote content type: {type(remote_content)}")

    async def _load_from_s3_async(
        self,
        content: Content,
        upsert: bool,
        skip_if_exists: bool,
    ...
    async def _load_from_gcs_async(
        self,
        content: Content,
        upsert: bool,
        skip_if_exists: bool,
    ...
    async def _load_from_sharepoint_async(
        self,
        content: Content,
        upsert: bool,
        skip_if_exists: bool,
    ...
    async def _load_from_github_async(
        self,
        content: Content,
        upsert: bool,
        skip_if_exists: bool,
        ...
        # TODO: Implement GitHub content loading using GitHub API or raw content URLs
        # This will require:
        # 1. Authenticate with GitHub using token (if private repo)
        # 2. Fetch file(s) from repo at file_path or folder_path
        # 3. Process through reader and insert to vector db
        log_warning(
            f"GitHub loading not yet implemented. Config: {gh_config.id}, repo: {gh_config.repo}, "
            f"file_path: {remote_content.file_path}, folder_path: {remote_content.folder_path}"
        )
